"""
Quick validation: random configs through STEP gen + 4 validators.
Run from PulleyWebApp-ss root:
    .venv312/Scripts/python.exe tests/run_quick_validate.py [seed [count]]

Validators:
  1. NIST SFA 5.45     (syntax check)
  2. OCP/OpenCASCADE   (BRepCheck + volume, via cadquery's OCP bindings)
  3. FreeCAD 1.1       (Part.read + solid count + volume, headless)
  4. eDrawings 2026    (COM load event, Windows only — skipped if no GUI context)
"""
import sys
import os
import tempfile
...
def validate_sfa(step_bytes):
    ...
    if not _SFA.is_file():
        return None, f"sfa-cl.exe not found at {_SFA}"
    ...
    try:
        ...
        probs = [l.strip() for l in lines if '**' in l]
        clean = any('No syntax errors or warnings' in l for l in lines)
...
def validate_occ(step_bytes):
    """OpenCASCADE BRepCheck + volume via cadquery OCP bindings."""
    if not _OCP_SITE.is_dir():
        return None, f"OCP site-packages not found at {_OCP_SITE}"
    ...
    try:
        ...
        if status != IFSelect_RetDone:
            return False, f"ReadFile failed (status={status})"
        ...
        if not analyzer.IsValid():
            return False, "BRepCheck: shape invalid"
        props = GProp_GProps()
...
def validate_freecad(step_bytes):
    """FreeCAD 1.1 headless: Part.read + solid count + volume."""
    if not _FC_CMD.is_file():
        return None, f"freecadcmd.exe not found at {_FC_CMD}"
    if not _FC_SCRIPT.is_file():
        return None, f"_freecad_import.py not found at {_FC_SCRIPT}"
    with tempfile.NamedTemporaryFile(suffix='.step', delete=False, mode='wb') as f:
        f.write(step_bytes)
        tmp = f.name
    try:
        ...
        # Filter out FreeCAD banner lines
        lines = [l for l in out.splitlines()
                 if l and not l.startswith('FreeCAD') and not l.startswith('(C)') and
                 not l.startswith('FreeCAD is free')]
        msg = lines[0] if lines else out[:200]
        if res.returncode == 0 and msg.startswith('OK:'):
            return True, msg
        if res.returncode == 2:
            return False, msg  # no solids
...
def validate_edrawings(step_bytes):
    """eDrawings 2026 COM load check (requires foreground Windows process)."""
    try:
        import pythoncom
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            if 'ok' in result:
                break
        if 'ok' not in result:
            return False, "timeout after 20s — no load event"
...
with flask_app.test_client() as c:
    for i in range(count):
        ...
        label = (f"{cfg['family']}-{cfg['pitch']}-{cfg['teeth']}T"
                 f" bore={cfg['bore']}"
        ...
        for name, fn in [
            ...
            ('eDrawings 2026 ', validate_edrawings),
        ]:
            ok, msg = fn(resp.data)
