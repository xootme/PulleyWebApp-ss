...
def random_configs():
    """Load pre-generated configs from disk (generated by run_tests.py before
    pytest started, avoiding import conflicts with the Flask background threads).
    Falls back to generating inline if no pre-generated file exists.
    """
    log_dir = Path(__file__).parent.parent / 'logs' / 'nightly_random'
    files   = sorted(log_dir.glob('*.json')) if log_dir.exists() else []
    if files:
        # Use most recent file generated today
        today = datetime.now().strftime('%Y-%m-%d')
        today_files = [f for f in files if f.name.startswith(today)]
        if today_files:
            data = json.loads(today_files[-1].read_text(encoding='utf-8'))
            configs = data['configs']
            print(f'\n[nightly] loaded {len(configs)} configs from {today_files[-1].name}')
            return configs
    ...
    print(f'\n[nightly] configs generated → {path}')
    return configs
