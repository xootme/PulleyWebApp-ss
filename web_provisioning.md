...
# Concurrency test (server must be running at 127.0.0.1:5000)
.venv312/Scripts/python concurrency_test.py --csv Perf_History.csv
```

`record_benchmarks.py` prints a summary table and appends one row per test to `Perf_History.csv`
tagged with the current commit hash and timestamp. Review the table for unexpected regressions
before proceeding — a 2× slowdown in an STL test is worth investigating.

`concurrency_test.py --csv` appends rows named `conc/<endpoint>/N<n>` with the serialisation
`ratio` column filled in (ratio ≈ 1.0 = good parallelism; ratio ≈ N = GIL/single-worker block).

**Checklist:**
- [ ] `record_benchmarks.py` ran without errors (exit 0)
- [ ] No test regressed more than ~30% vs the previous entry in `Perf_History.csv`
- [ ] `concurrency_test.py --csv` ran without errors
- [ ] No new `SERIALISED [!!]` flags that weren't present before
